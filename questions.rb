...
def select_elements_starting_with_vowel(array)
	array.select { |element| element.chars.first =~ /[aeiou]/ }
end
...
def get_first_half_of_string(string)
	first_half = string.size / 2.0
	string.slice(0...first_half.ceil)
end
...
def number_of_elements_that_are_palindromes(array)
	array.select { |element| element.reverse == element }.count
end
...
def is_a_3_dot_range?(range)
	return true if range.to_s.match(/[.]{3}/)
	false
end
...
def word_count_a_file(file_path)
	file = File.open(file_path, 'r')
		total_of_words = file.readlines.first.split(" ").count
	file.close
	total_of_words
end
...
def your_birthday_is_on_a_friday_in_the_year(birthday)
	date = birthday + (60*60*24*365)
	loop do
		date = date + (60*60*24*365)
		return date.year if (date).friday?
	end
end
...
def count_words_of_each_length_in_a_file(file_path)
	words_times = Hash.new(0)
	file = File.open(file_path, 'r')
		each_word = file.readlines.first.split(' ').map { |word| word.gsub(/\W/, '') }.map { |word| word.length }.each {|size| words_times[size] += 1 }
	file.close
	words_times
end
